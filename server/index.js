...
glob('/repo_files/*.template').forEach(file => {
...
app.use('/browse/repo_files',express.static('/repo_files'))
...
const modCache = (req,res,next) => {
    ...
    switch(t.type){
        case 'pip':
            ...
            break
        case 'docker':
            const registry = DOCKER.registries.find(r => req.path.includes(r))
            opts.url = registry ? 'https://'+registry+'/'+req.path.replace(registry+'/','') : DOCKER.url + req.path
